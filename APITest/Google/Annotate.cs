...
using OCRAPITest.LineSegmentation;
        ...
        public async Task<string>  GetText(string imgPath,string language,string type)
        {
            ...
            if (batchAnnotateImagesResponse.Responses.Any())
            {
                ...
                else 
                {
                    ...
                    // line segmentation for invoice 

                    var resultAfterLineSegmentation = GoogleVisionLineSegmentation.GetLineSegmentation(annotateImageResponse.TextAnnotations as IReadOnlyList<EntityAnnotation>);
