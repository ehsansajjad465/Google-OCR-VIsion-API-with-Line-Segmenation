...
namespace OCRAPITest.LineSegmentation
{
    public class GoogleVisionLineSegmentation
    {
        ...
        static List<ReceiptEntityAnnotation> Initialization(IReadOnlyList<EntityAnnotation> data)
        {
            ...
            int yMax = data[0].BoundingPoly.Vertices.Max(y => Convert.ToInt32(y.Y));
        ...
        static string ArrangeWordsInOrder(List<ReceiptEntityAnnotation> mergedArray, int k)
        {
            ...
            for (int i = 0; i < line.Count; i++)
            {
                ...
                int mainX = Convert.ToInt32(mergedArray[k].Vertices[0].X);
                int compareX = Convert.ToInt32(mergedArray[index].Vertices[0].X);
